...
import { Float, TorusKnot, MeshTransmissionMaterial, Environment } from "@react-three/drei";
...
function FloatingGeometry() {
  ...
  return (
    <Float speed={1.4} rotationIntensity={0.4} floatIntensity={0.8}>
      <TorusKnot ref={meshRef} args={[1, 0.32, 180, 24]}>
      ...
      <TorusKnot args={[1.08, 0.34, 180, 24]}>
        <meshStandardMaterial
          color="#ffd700"
          transparent
          opacity={0.06}
...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (storageError) {
        throw new Error("File upload failed. Try again.");
      }
      ...
      if (dbError) {
        ...
        throw new Error("Failed to record submission. Try again.");
      }
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          >
            <p className="text-white/50 text-sm leading-relaxed mb-8">
              Open to all VIT students. Participation in the learning session is mandatory before the competition. Judging begins immediately after drafting time closes. Please upload your document in PDF or DOCX format.
            </p>
                ...
                <motion.div key="form" exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
                  <div className="border border-white/10 bg-white/[0.02] p-6 space-y-4">
                    ...
                    <button
                      ...
                      disabled={uploading}
                      className="group relative w-full font-mono text-sm tracking-widest uppercase py-3 border border-yellow-400 text-yellow-400 overflow-hidden hover:text-black hover:bg-yellow-400 transition-colors duration-300 mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <span className="relative">{uploading ? "UPLOADING..." : "SUBMIT REGISTRATION"}</span>
                    </button>
                  </div>
                </motion.div>
              ) : (
